...
const LEAD_IN = '\\s*[:–—-]?\\s*(?:ca\\.?)?\\s*(?:kl\\.?)?\\s*';

// "Live från ca 20.00", "Live från: 20.00", "LIVE FRÅN KL 19.45"
const SHOWTIME = new RegExp(`live(?:\\s*fr[åa]n)?${LEAD_IN}${TIME}`, 'i');
// "Insläpp 19.00", "Insläpp: 19.00", "Dörrar: 19.00"
...
export function parseSwedishEventDate(text, { now = Date.now(), timeZone = ZONE } = {}) {
  ...
  const showtime = matchTime(text, SHOWTIME_TRAILING, SHOWTIME);
